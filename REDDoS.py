...
class bcolors:
    HEADER = '\033[38;5;129m'   # Светлый фиолетовый
    OKBLUE = '\033[38;5;135m'   # Лавандовый
    OKCYAN = '\033[38;5;141m'  # Бледно-синий с фиолетовым оттенком
    OKGREEN = '\033[38;5;147m' # Бледный фиолетовый
    WARNING = '\033[38;5;153m' # Лиловый
    FAIL = '\033[38;5;159m'    # Яркий фиолетовый
    RESET = '\033[0m'
    BOLD = '\033[1m'
    UNDERLINE = '\033[4m'
...
class Methods:
    LAYER7_METHODS: Set[str] = {
        ...
        "APACHE", "XMLRPC", "BOT", "DOWNLOADER", "KILLER", "TOR", "RHEX", "STOMP"
    }
    ...
    def usage():
        while True:
            ...
            print((
                ...
                '             `9MMP\'     \n'
                f'{bcolors.RESET}\n'
                '  | Leaked in https://t.me/pr0xit\n'
                f'{bcolors.HEADER}{bcolors.BOLD}𝙱𝚈: 𝚁𝙴𝙽𝙲(@𝚇𝚊𝚑𝚛𝚟𝚜) | ＲＥ: ＤＤｏＳ - DDoS Attack Tool Featuring {len(Methods.ALL_METHODS) + 3 + len(ToolsConsole.METHODS)} Methods{bcolors.RESET}\n'
                f'{bcolors.OKBLUE}⚠️ 𝐍𝐨𝐭𝐞: 𝐖𝐢𝐭𝐡𝐨𝐮𝐭 𝐚 𝐩𝐫𝐨𝐱𝐲 𝐥𝐢𝐬𝐭, 𝐭𝐡𝐞 𝐚𝐭𝐭𝐚𝐜𝐤 𝐰𝐢𝐥𝐥 𝐫𝐮𝐧 𝐰𝐢𝐭𝐡𝐨𝐮𝐭 𝐩𝐫𝐨𝐱𝐲𝐞𝐬.{bcolors.RESET}\n'
                f'{bcolors.OKCYAN}📥 𝐌𝐢𝐬𝐬𝐢𝐧𝐠 𝐩𝐫𝐨𝐱𝐲 𝐟𝐢𝐥𝐞? 𝐓𝐡𝐞 𝐬𝐜𝐫𝐢𝐩𝐭 𝐰𝐢𝐥𝐥 𝐚𝐮𝐭𝐨-𝐝𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐚𝐧𝐝 𝐯𝐚𝐥𝐢𝐝𝐚𝐭𝐞 𝐩𝐫𝐨𝐱𝐲𝐞𝐬.{bcolors.RESET}\n'
                f'{bcolors.OKGREEN}🔎 𝐏𝐫𝐨𝐱𝐲 𝐓𝐲𝐩𝐞 𝟎 - 𝐈𝐧𝐜𝐥𝐮𝐝𝐞𝐬 𝐚𝐥𝐥 𝐩𝐫𝐨𝐱𝐲𝐞𝐬 𝐥𝐢𝐬𝐭𝐞𝐝 𝐢𝐧 𝐜𝐨𝐧𝐟𝐢𝐠.𝐣𝐬𝐨𝐧{bcolors.RESET}\n'
                f'\n'
                f'{bcolors.WARNING}🛠️ SocksTypes:{bcolors.RESET}\n'
                f'|{bcolors.FAIL}  - 6 = RANDOM{bcolors.RESET}\n'
                f'|{bcolors.HEADER}  - 5 = SOCKS5{bcolors.RESET}\n'
                f'|{bcolors.OKBLUE}  - 4 = SOCKS4{bcolors.RESET}\n'
                f'|{bcolors.OKCYAN}  - 1 = HTTP{bcolors.RESET}\n'
                f'|{bcolors.OKGREEN}  - 0 = ALL{bcolors.RESET}\n'
                f'\n'
                f'{bcolors.WARNING}💣 Methods:{bcolors.RESET}\n'
                f'{bcolors.FAIL} - Layer4{bcolors.RESET}\n'
                f'{bcolors.HEADER} | {", ".join(Methods.LAYER4_METHODS)} | {len(Methods.LAYER4_METHODS)} Methods{bcolors.RESET}\n'
                f'{bcolors.OKBLUE} - Layer7{bcolors.RESET}\n'
                f'{bcolors.OKCYAN} | {", ".join(Methods.LAYER7_METHODS)} | {len(Methods.LAYER7_METHODS)} Methods{bcolors.RESET}\n'
                f'{bcolors.OKGREEN} - Tools{bcolors.RESET}\n'
                f'{bcolors.WARNING} | {", ".join(ToolsConsole.METHODS)} | {len(ToolsConsole.METHODS)} Methods{bcolors.RESET}\n'
                f'{bcolors.FAIL} - Others{bcolors.RESET}\n'
                f'{bcolors.HEADER} | {", ".join(["TOOLS", "HELP", "STOP", "EXIT"])} | {3} Methods{bcolors.RESET}\n'
                f'{bcolors.OKBLUE} - All {len(Methods.ALL_METHODS) + 3 + len(ToolsConsole.METHODS)} Methods{bcolors.RESET}\n'
                f'\n'
                f'{bcolors.OKCYAN}\n🔍 Example Usage:{bcolors.RESET}\n'
                f' |L7 python3 {argv[0]} <method> <url> <socks_type> <threads> <proxylist> <rpc> <duration> <debug=optional>{bcolors.RESET}\n'
                f' |L4 python3 {argv[0]} <method> <ip:port> <threads> <duration>{bcolors.RESET}\n'
                f' |L4 Proxied: python3 {argv[0]} <method> <ip:port> <threads> <duration> <socks_type> <proxylist>{bcolors.RESET}\n'
                f' |L4 Amplification: python3 {argv[0]} <method> <ip:port> <threads> <duration> <reflector file>{bcolors.RESET}\n'
                '  | Leaked in https://t.me/pr0xit'
            ))

            command = input(f"{bcolors.OKGREEN}Enter command (or type 'exit' to quit):{bcolors.RESET} ").strip()
            if command.lower() == 'exit':
                print(f"{bcolors.OKGREEN}Exiting...{bcolors.RESET}")
                break
            else:
                print(f"{bcolors.WARNING}Unknown command: {command}{bcolors.RESET}")
                ...
                if len(argv) >= 6:
                    ...
                    if argfive:
                        ...
                        if method in {"NTP", "DNS", "RDP", "CHAR", "MEM", "CLDAP", "ARD"}:
                            ...
                            if method not in {"TCP", "CPS", "CONNECTION"}:
